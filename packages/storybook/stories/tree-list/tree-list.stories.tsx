...
const ComprehensiveTreeList = observer(() => {
  ...
  const handleNodeDrop = (
    ...
    try {
      ...
      const findNodePath = (
        ...
        for (let i = 0; i < nodes.length; i++) {
          ...
          if (nodes[i].children?.length) {
            const foundPath = findNodePath(nodes[i].children!, id, [
              ...path,
              i,
              "children",
            ]);
        ...
        for (let i = 0; i < targetPath.length; i += 2) {
          if (i % 2 === 0) {
            ...
            if (node) {
              ...
              current =
                i + 1 < targetPath.length && node.children ? node.children : [];
            }
          }
        }
      ...
      const insertNodeAtPath = (
        ...
        if (position === "inside") {
          // 向节点内部插入
          let current: TreeNodeData[] | TreeNodeData | null | undefined =
            result;
          for (const segment of path) {
          ...
          for (let i = 0; i < path.length - 1; i++) {
            ...
            if (Array.isArray(current)) {
              current =
                typeof segment === "number" ? current[segment] : current;
            } else if (current && typeof current === "object") {
              current =
                segment === "children" && current.children
                  ? current.children
          ...
          if (typeof lastSegment === "number") {
            if (Array.isArray(current)) {
              // 计算插入位置
              const insertIndex =
                position === "before" ? lastSegment : lastSegment + 1;
  ...
  const selectSpecificNodeExternally = useCallback(() => {
    ...
    const allAncestorsExpanded =
      ...
      ancestorIds.every((id) => expandedNodeIds.has(id));
        ...
        <div className="relative h-full w-full">
          ...
          <ContextMenu triggerRef={contextMenuTriggerRef}>
            <ContextMenu.Content className="min-w-[220px]">
              ...
              <ContextMenu.Item
                onClick={() => handleContextMenuAction("Add child under")}
              >
        ...
        <div className="bg-light-50 flex h-screen min-h-0 w-full flex-1 flex-col">
          <div className="flex flex-col gap-3 p-6">
            ...
            <div className="border-default-border bg-default-background text-body-small text-secondary-foreground rounded border p-3">
              <div className="text-default-foreground font-medium">
                Last hover
              </div>
              {lastHoveredNode ? (
                <div className="mt-1 space-y-1">
                  ...
                  <div>
                    Status: {lastHoveredNode.isHovered ? "hovered" : "idle"}
                  </div>
                  <div className="truncate">
                    Path:{" "}
                    {lastHoveredNode.path.length > 0
                      ? lastHoveredNode.path.join(" / ")
                      : "—"}
                  </div>
                </div>
              ) : (
            ...
            <div className="border-default-border bg-default-background text-body-small text-secondary-foreground rounded border p-3">
              ...
              <div className="text-default-foreground mt-1">
                {lastActionLog}
              </div>
              <div className="text-body-small text-tertiary-foreground mt-1">
...
const LabelVisibilityDemo = () => {
  ...
  const renderLabel = useCallback(
    (node: TreeNodeType) => (
      <span className="text-body-small text-default-foreground font-medium">
        {node.name}
      </span>
    ),
    [],
    ...
    <div className="flex flex-col gap-6 lg:flex-row">
      <div className="border-default-border bg-default-background rounded border p-4 shadow-sm">
        <div className="text-body-small text-secondary-foreground mb-3">
          Hovering nodes with actions hides the floating label and reveals the
          buttons.
        </div>
        <div className="w-[320px]">
          <TreeList
            ...
            onNodeSelect={(nodes) =>
              handleSelectionChange(nodes, setWithActionsSelection)
            }
      ...
      <div className="border-default-border bg-default-background rounded border p-4 shadow-sm">
        ...
        <div className="w-[320px]">
          <TreeList
            ...
            onNodeSelect={(nodes) =>
              handleSelectionChange(nodes, setPlainSelection)
            }
            renderLabel={renderLabel}
            renderIcon={renderIcon}
            allowDrag={false}
          ...
          <div className="flex flex-col gap-3 p-6">
            ...
            <div className="border-default-border bg-default-background text-body-small text-secondary-foreground rounded border p-3">
              ...
              <div className="mt-2 space-y-2">
                ...
                <div>
                  Virtual scroll: {virtualScroll ? "enabled" : "disabled"}
                </div>
                <div>Selected nodes: {selectedNodeIds.size}</div>
              </div>
            </div>
            <div className="text-body-small text-tertiary-foreground mt-2">
              Toggle virtual scrolling to compare rendering behaviour with a 10k
              node dataset.
            </div>
          </div>
        </div>
