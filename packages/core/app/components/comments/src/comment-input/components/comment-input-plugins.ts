  ...
  editor.deleteBackward = (unit) => {
    ...
    if (selection && Range.isCollapsed(selection)) {
      try {
        ...
        const prevEntry = Editor.previous(customEditor)

        if (prevEntry && isMentionElement(prevEntry[0])) {
          // 删除提及节点
          Transforms.removeNodes(customEditor, { at: prevEntry[1] })
          return
        }
      } catch (error) {
  ...
  editor.deleteForward = (unit) => {
    ...
    if (selection && Range.isCollapsed(selection)) {
      try {
        // 检查下一个节点是否是提及
        const customEditor = editor as unknown as CustomEditor
        const nextEntry = Editor.next(customEditor)

        if (nextEntry && isMentionElement(nextEntry[0])) {
          // 删除提及节点
          Transforms.removeNodes(customEditor, { at: nextEntry[1] })
          return
        }
      } catch (error) {
