# Changelog

All notable changes to the Comments component will be documented in this file.

## [2025-08-25]

### Fixed

- Fix README formatting and standardize className ordering
- Increase stdout maxBuffer to prevent state retrieval failures

## [2025-08-22]

### Changed

- Update component and hook READMEs; improve format-date utils

## [2025-08-13]

### Added

- Add README docs for components and hooks; refactor hook structure

## [2025-07-13]

### Changed

- Refactor Storybook global styles and update component stories

## [2025-07-10]

### Changed

- Remove Storybook addon-storysource and update component stories
- Move experimental comments components to main comments directory

## [2025-05-09]

### Changed

- Enhance component structure and update dependencies for improved usability

## [2025-05-08]

### Added

- Add new configuration files and components for improved project structure

## [Earlier]

### Changed

- Update package version to 0.2.1 and refactor comments and input components
- Enhance comment input and item components with new features and refactor structure
- Refactor comment input and item components to improve type definitions and imports

### Added

- Implement comment input component with emoji and mention support
