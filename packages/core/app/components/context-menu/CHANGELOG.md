# Changelog

All notable changes to the Context Menu component will be documented in this file.

## [2025-12-06]

### Added

- Add triggerSelector support for external element binding

## [2025-09-02]

### Changed

- Improve context menu behavior and add nested menu tests
- Add SimpleDropdownNested story for ContextMenu component

## [2025-08-25]

### Fixed

- Fix README formatting and standardize className ordering
- Increase stdout maxBuffer to prevent state retrieval failures

## [2025-08-22]

### Changed

- Update component and hook READMEs; improve format-date utils

## [2025-08-13]

### Added

- Add README docs for components and hooks; refactor hook structure

## [2025-07-13]

### Changed

- Refactor Storybook global styles and update component stories

## [2025-07-10]

### Changed

- Remove Storybook addon-storysource and update component stories

## [2025-07-09]

### Fixed

- Fix context menu nesting conflicts using `triggerRef` for better component integration

## [2025-05-09]

### Changed

- Enhance component structure and update dependencies for improved usability

### Added

- Add disabled state support to ContextMenu component

## [2025-05-08]

### Added

- Add new configuration files and components for improved project structure
- Add context menu components and utilities
